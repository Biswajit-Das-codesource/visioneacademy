...
import data2023 from '../Jsondata/Railway_Group_D_2023_Questions.json';

import { arrayUnion, doc, updateDoc } from "firebase/firestore";
import { db } from "../config/firebase"; 
import { getAuth } from "firebase/auth";

function Railway_GD_2023() {
    const [selectedOptions, setSelectedOptions] = useState({});
    const [successCount, setSuccessCount] = useState(0);
    const [showResult, setShowResult] = useState(false);
    const [showDialog, setShowDialog] = useState(false);
  
    const questions = data2023.questions;
    ...
    const handleSubmit = () => {
     ...
     const performSubmission = async () => {
      ...
      // 🔥 Save only score info to Firestore
      if (user) {
        try {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center p-4">
      <h1 className="text-3xl font-bold text-blue-600 mb-6">{data2023.exam} Quiz</h1>
      <div className=" rounded-lg p-6 w-full max-w-4xl">
        {!showResult ? (
          <>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">Total Questions: {totalQuestions}</h2>
              ...
              return (
                <div
                  ...
                  className={`mb-6 p-4 rounded-lg ${
                    isSkipped ? 'border-2 border-gray-100' : 'border border-gray-300'
                  }`}
                >
                  {question.direction && (
                    <p className="text-md text-gray-600 mb-3 whitespace-pre-wrap">
                      <span className="font-semibold">Direction:</span> {question.direction}
                    </p>
                  )}
              ...
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                <div className="bg-white rounded-lg p-6 max-w-md w-full">
                  ...
                  <p className="text-gray-600 mb-6">
                    You have skipped {skippedCount} question{skippedCount > 1 ? 's' : ''}. Please
                    review them before submitting, or confirm to submit now.
                  </p>
          ...
          <div className="text-center">
            <h2 className="text-2xl font-bold text-blue-600 mb-4">Quiz Results</h2>
            <div className="mb-6">
            ...
            <div className="mb-8">
              {questions.map((question, questionIndex) => {
                ...
                return (
                  ...
                  >
                    {question.direction && (
                      <p className="text-md text-gray-600 mb-2 whitespace-pre-wrap">
                        <span className="font-semibold">Direction:</span> {question.direction}
                      </p>
                    )}
                    <p className="font-medium text-gray-800">
                      Q{questionIndex + 1} ({question.section}): <br/>{question.question}
                    </p>
                    <p
                      className={`mt-2 ${
                        answer
...
export default Railway_GD_2023
