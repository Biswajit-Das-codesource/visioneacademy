...
import data2023 from "../Jsondata/SBI Clerk Mains Memory Based 2023-2024 Questions.json";
import { arrayUnion, doc, updateDoc } from "firebase/firestore";
import { db } from "../config/firebase";
import { getAuth } from "firebase/auth";

function Sbi_cleark_2023_24() {
  const [selectedOptions, setSelectedOptions] = useState({});
  const [successCount, setSuccessCount] = useState(0);
  const [showResult, setShowResult] = useState(false);
  const [showDialog, setShowDialog] = useState(false);

  const questions = data2023.exam.questions;
  // console.log(questions.length)
  const totalQuestions = data2023.exam.questions.length;
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center p-4">
      <h1 className="text-3xl font-bold text-blue-600 mb-6">
        SBI Clerk Mains 2023-2024 Quiz
      </h1>
      <div className="rounded-lg  p-6 w-full max-w-4xl">
        {!showResult ? (
          <>
            ...
            {data2023.exam.questions.map((question, questionIndex) => {
...
export default Sbi_cleark_2023_24;
